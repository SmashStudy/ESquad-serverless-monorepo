    ...
    try {
        ...
        for (const record of event.Records) {
            ...
            if (connectionIds.length === 0) {
                console.warn(`No active connections found for userId ${sendNotification.userId}`);
                continue;
            }

            // Send notification to each connection
            await Promise.all(connectionIds.map((id) => sendToConnection(id, sendNotification)));
        }
