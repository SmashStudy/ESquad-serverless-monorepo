...
describe('handleOptions 함수', () => {
  test('handleOptions는 올바른 CORS 응답 객체를 반환해야 합니다', () => {
  ...
  test('handleOptions는 응답 객체의 body가 빈 문자열이어야 합니다', () => {
    const response = handleOptions();
    expect(response.body).toBe('');
  });

  test('handleOptions는 응답 객체의 statusCode가 200이어야 합니다', () => {
    const response = handleOptions();
    expect(response.statusCode).toBe(200);
  });

  test('handleOptions는 응답 객체에 CORS_HEADERS가 포함되어야 합니다', () => {
    const response = handleOptions();
