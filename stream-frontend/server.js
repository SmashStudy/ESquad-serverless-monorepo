...
app.post("/stream/join", async (req, res) => {
  try {
    const { title, attendeeName, region = "us-east-1", ns_es } = req.body;

    // URL 디코딩
    const decodedAttendeeName = decodeURIComponent(attendeeName);
    ...
    attendeeCache[title][Attendee.AttendeeId] = {
      ...Attendee,
      Name: decodedAttendeeName, // 디코딩된 이름 사용
    };
...
app.get("/stream/attendee", (req, res) => {
  try {
